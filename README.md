...
对于二分类问题，其实也是可以用MSE来做cost function的，对于两层的神经网络（不算输入层），如果第一层的activation不用sigmoid function，而用$f(x)=x$，第二层的activation用sigmoid function，cost function用MSE来算（正样本结果应该比较接近1，负样本结果应该比较接近0），做back propagation时得到的gradient的结果也与上面的cost function用作二分类的结果一样，只是在实际上，没必要这么做，就用上面的cost function就好了。
...
需要注意的是，如果是全量梯度下降，learning rate一定要很小（特别是没用mean square error的时候），否则系数可能变化太大导致产生数值计算中的负无穷大或者正无穷大。如果是全量梯度下降，因为每个iteration权重才更新一次，实际上是会影响收敛的速度的，可能不如随机梯度下降的收敛速度快，甚至可能要慢的多。
